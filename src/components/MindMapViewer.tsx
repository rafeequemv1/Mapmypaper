  ...
  useEffect(() => {
    if (isMapGenerated && containerRef.current && !mindMapRef.current) {
      ...
      const options = {
        ...
        nodeMenu: true,
        tools: {
          zoom: false, // Disable default zoom since we'll add our own
          create: true,
          edit: true,
          layout: false, // We'll add our own layout controls
        },
        theme: colorfulTheme,
        autoFit: true,
      ...
      customNodeMenu.menus = (node: any, mind: MindElixirInstance) => {
        ...
        // Add explain option to trigger chat
        menus.push({
          name: '💬 Explain Node',
          onclick: () => {
        ...
        topicElements.forEach((element) => {
          element.addEventListener('click', (e) => {
            ...
            if (mind.currentNode && mind.currentNode.nodeObj) {
              ...
              // Trigger explain functionality when node is clicked
              if (onExplainText && mind.currentNode.nodeObj.topic) {
                onExplainText(mind.currentNode.nodeObj.topic);
                onRequestOpenChat?.();
              }
            }
          });
      ...
      // Dispatch a global event with the mind map instance for other components
      const event = new CustomEvent('mindMapReady', { 
        detail: { mindMap: mind } 
      });
      ...
      toast({
        ...
        description: "Click on any node to explain it. Right-click for more options.",
        duration: 5000,
      });
