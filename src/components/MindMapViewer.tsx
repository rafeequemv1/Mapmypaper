...
const MindMapViewer = ({ 
  ...
  const generateNodeSummary = (nodeData: any) => {
    ...
    const countNodes = (node: any): number => {
      ...
      if (node.children && node.children.length > 0) {
        // Add the count of all children nodes
        for (const child of node.children) {
    ...
    const nodeText = totalNodes === 1 ? "node" : "nodes";
    
    // Add the hierarchical structure to the summary
    summaryText += extractTopics(nodeData);
    
    // Add statistics footer
    summaryText += `\n**Summary Statistics:**\n`;
    summaryText += `- Total ${nodeText}: ${totalNodes}\n`;
    
    // Display the summary
    setSummary(summaryText);
    setShowSummary(true);
    
    // Send the summary to the chat if available
    if (onExplainText) {
      onExplainText(summaryText);
      
      // Open the chat panel if handler is provided
      if (onRequestOpenChat) {
        onRequestOpenChat();
      }
    } else {
      // Show a toast if chat function is not available
      toast({
        title: "Summary Generated",
        description: "Summary created but chat panel is not available.",
      });
    }
  };

  // Display loading skeleton if map generation is in progress
  if (isLoading) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-gray-50">
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="space-y-3">
            ...
            {/* First level nodes skeleton */}
            <div className="flex flex-wrap justify-center gap-4 mt-6">
              {Array(3).fill(0).map((_, i) => (
                <Skeleton key={`node-1-${i}`} className="h-10 w-32 rounded-lg" />
              ))}
            </div>
            
            {/* Second level nodes skeleton */}
            <div className="flex flex-wrap justify-center gap-3 mt-4">
              {Array(5).fill(0).map((_, i) => (
                <Skeleton key={`node-2-${i}`} className="h-8 w-28 rounded-md" />
              ))}
            </div>
          </div>
          ...
          {loadingProgress > 0 && (
            <div className="space-y-2">
              ...
              <p className="text-center text-sm text-gray-500">Building mind map...</p>
            </div>
          )}
        </div>
      </div>
  ...
  // Display message if map generation hasn't started
  if (!isMapGenerated) {
    return (
      <div className="w-full h-full flex items-center justify-center p-8 bg-gray-50">
        <div className="text-center max-w-md mx-auto space-y-4">
          <FileText className="h-16 w-16 text-gray-400 mx-auto" />
          <h3 className="text-xl font-semibold text-gray-700">Mind Map Viewer</h3>
          <p className="text-gray-500">Upload a PDF or document to generate a mind map from its contents.</p>
          
          {/* Added a waiting state with animated loader */}
          {isLoading && (
            <div className="flex items-center justify-center space-x-2 mt-4">
              <LoaderCircle className="h-5 w-5 text-primary animate-spin" />
              <span className="text-gray-600">Preparing mind map...</span>
            </div>
          )}
  ...
  // Render the mind map container when ready
  return (
    <div 
