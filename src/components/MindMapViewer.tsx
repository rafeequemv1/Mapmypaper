...
const MindMapViewer = ({ 
  ...
  const generateNodeSummary = (nodeData: any) => {
    ...
    const depth = (function calculateDepth(node: any, currentDepth: number = 0): number {
    ...
    summaryText += `**Nodes:** ${totalNodes}\n`;
    summaryText += `**Depth:** ${depth}\n\n`;
    
    // Add hierarchical topic structure
    summaryText += "### Structure\n\n";
    ...
    toast({
      ...
      description: `Created summary for "${nodeData.topic.substring(0, 30)}${nodeData.topic.length > 30 ? '...' : ''}"`,
    });
  ...
  // Render loading state if mindmap is being generated
  if (isLoading || loadingProgress > 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        <div className="w-full max-w-md space-y-4">
          <div className="text-center mb-6">
            <h3 className="text-lg font-medium">Generating Mind Map</h3>
            <p className="text-sm text-muted-foreground">Please wait while we organize your content</p>
          </div>
  ...
  // Render placeholder if mind map data is not yet generated
  if (!isMapGenerated) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 bg-slate-50/50 dark:bg-slate-900/20 border border-dashed rounded-lg">
        <FileText className="h-12 w-12 text-slate-400 mb-4" />
        <h3 className="text-lg font-medium mb-2">No Mind Map Generated</h3>
        <p className="text-sm text-center text-slate-600 dark:text-slate-400 max-w-md mb-6">
          A mind map will be generated automatically when you upload a document or click the "Generate Mind Map" button in the chat.
        </p>
        <Button 
          variant="outline"
          onClick={() => {
            if (onExplainText) {
              onExplainText("Please generate a mind map for this document");
            }
            if (onRequestOpenChat) {
              onRequestOpenChat();
            }
